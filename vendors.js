...
const Vendors = ({navigation, route}) => {
    ...
    const [vtype,setVtype]=useState("Decorators & Florists")
    const [showDropDown, setShowDropDown] = useState(false);
    const dlist=[
        {
            label:"Decorators & Florists",
            value:"Decorators & Florists",
        },
        {
            label:"Caterers",
            value:"Caterers",
        },
        {
            ...
            value:"Venue Managers",
        },
        {
            label:"Photographers & Videographer",
            value:"Photographers & Videographer",
        },
        {
            label:"Entertainment",
            value:"Entertainment",
    ...
    useEffect(()=>{
        ...
        setEvent(route.params)
           ...
           const temp=data.slice(0,25).map((c)=>{c.check=route.params.vendors.map((e)=>e.phoneNumbers[0].number).includes(c.phoneNumbers[0].number)
            return c
            })
            setContacts(temp)
    ...
    return !event ? (<View></View>):( 
        ...
        <DropDown label="Vendor Types" mode="outlined" value={vtype} setValue={setVtype} list={dlist}
        ...
        <ScrollView style={{
            ...
            {event.vendors.filter((v)=>v.vtype==vtype).map((g,i)=>(
                <Card key={i}
                style={{
                    marginBottom:15,
        ...
        left={<TextInput.Icon icon={'magnify'}color="#796299"></TextInput.Icon>}>
            ...
            {searchContacts.filter((v)=>!v.vtype || v.vtype==vtype).map((c,i)=>(
                <Card key={i} style={{
                    marginBottom:15,
                    backgroundColor:"#FEF0ED",
                    elevation:0,
                ...
                <Checkbox status={c.check && c.vtype==vtype ? 'checked' : 'unchecked'}
                    onPress={() => {
                        ...
                        setContacts(contacts.map((e)=>{
                            ...
                            {
                                if (e.vtype && e.vtype==vtype)
                                    e.check=!e.check
                                else
                                // if(e.vtype && e.vtype!=v.type)
                                    e.check=true
                                    e.vtype=e.check ? vtype : null
            ...
            <Button mode="contained" onPress={()=>{
                const selectedContacts=contacts.filter((e)=>e.check)
                    ...
                    storage.load({
                        ...
                        data[ind].vendors=selectedContacts
